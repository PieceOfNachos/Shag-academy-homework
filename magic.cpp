...
int get_mana_spend() {//функция-геттер для затрата маны
    return this->mana_spend;
}

int get_education_level() {//функция-геттер для уровня образованности
    return this->education_level;
}

int level_of_magic() { //функция-геттер для уровня магии
    return this->level_of_magic;
}

int get_cost_of_education() { //функция-геттер для цены на образования 
...
int war_magic::use_magic_to_heal(){
    srand(time(0));
